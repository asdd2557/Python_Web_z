...
class Menuname(models.Model):
    ...
    menu1 = models.SlugField(max_length=200, allow_unicode=True,
                             blank=True)  # SlugField는 기본적으로 한글을 지원하지 않아서 allow_unicode = True는 한국어를 사용할 수 있게해주는 언어다.
    menu2 = models.SlugField(max_length=200, allow_unicode=True,
                             blank=True)  # SlugField는 기본적으로 한글을 지원하지 않아서 allow_unicode = True는 한국어를 사용할 수 있게해주는 언어다.
    menu3 = models.SlugField(max_length=200, allow_unicode=True,
                             blank=True)  # SlugField는 기본적으로 한글을 지원하지 않아서 allow_unicode = True는 한국어를 사용할 수 있게해주는 언어다.
    menu4 = models.SlugField(max_length=200, allow_unicode=True,
                             blank=True)  # SlugField는 기본적으로 한글을 지원하지 않아서 allow_unicode = True는 한국어를 사용할 수 있게해주는 언어다.
...
class Post(models.Model):
    ...
    category = models.ForeignKey(Category, null=True, blank=True,
                                 on_delete=models.SET_NULL)  # blank 는 공백이라는 뜻이므로 blank = True는 공백을 허가하겠다는 뜻이다. null은 추후에 value가 삭제될시에 null로 변환을 허용한다는 뜻
    ...
    def get_content_rich_text_field(self):  ##get_content_richtextfild 이미지 텍스트 리턴
        return self.content
